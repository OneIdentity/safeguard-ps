...
$local:SessionsLicense = (Install-SafeguardLicense -Appliance $local:Appliance -AccessToken $local:AccessToken -Insecure $SessionsLicenseFile -StageOnly)

$local:Licenses = (Get-SafeguardLicense -Appliance $Appliance -AccessToken $AccessToken -Insecure)
if ($local:Licenses.Count -lt 2)
{
    throw "Expected to see at least 2 licenses returned from Safeguard"
}
...
Install-SafeguardLicense -Appliance $Appliance -AccessToken $AccessToken -Insecure -Key $local:PasswordLicense.Key | Out-Null
